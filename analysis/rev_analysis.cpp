...
void REVfunc::RevAnalysis(ScaLBL_MRTModel &MRT) {
    ...
    while (x_side < Nx - 2) {
        ...
        double current_perm = h * h * mu * current_poro * flow_rate / force_mag; //permeability

        // double current_tort = 1e100;

        // if (vaz != 0)
        //     current_tort = sqrt(vax * vax + vay * vay + vaz * vaz) / vaz - 1;
        ...
        if (iter_step >= 1 && iter_step < samples - 1 && poro[iter_step] > 0)
        {
            mean = 0;
        ...
        if (iter_step >= samples - 1 && poro[iter_step] > 0) {
        ...
        if (RE < gamma && CC < gamma && rev_x_poro == -1) {
            rev_x_poro = x_side * MRT.Dm->voxel_length;
        }
        ...
        if (rev_x_poro != -1) {
            ...
            for (int l = 0; l < stat_samples; l++) {
                double stat_current_pore_count = 0;

                int start_x = 1 + rand() % (Nx - static_cast<int>(sub_x_size[iter_step]) - 2);
                int start_y = 1 + rand() % (Ny - static_cast<int>(sub_x_size[iter_step]) * Ny / Nx - 2);
                int start_z = 1 + rand() % (Nz - static_cast<int>(sub_x_size[iter_step]) * Nz / Nx - 2);

                int end_x = start_x + static_cast<int>(sub_x_size[iter_step]);
                int end_y = start_y + static_cast<int>(sub_x_size[iter_step]) * Ny / Nx;
                int end_z = start_z + static_cast<int>(sub_x_size[iter_step]) * Nz / Nx;
                ...
                stat_poro[l] = stat_current_pore_count / (sub_x_size[iter_step] * sub_x_size[iter_step] * (Ny / Nz) * sub_x_size[iter_step] * (Nz / Nx));
            }
        ...
        log_file = fopen("rev_analysis.csv", "a");
        fprintf(log_file, "%d %f %f %f %f %f %f %f %f %f %f ", iter_step, average_pore_size * MRT.Dm->voxel_length, sub_x_size[iter_step] * MRT.Dm->voxel_length,
        ...
        if (iter_step >= 1 && iter_step < (samples - 1) && perm[iter_step] > 0)
        {
        ...
        if (iter_step >= samples - 1 && perm[iter_step] > 0)
        {
        ...
        if (RE < gamma && CC < gamma && rev_x_perm == -1) {
            rev_x_perm = x_side * MRT.Dm->voxel_length;
        }
        ...
        if (rev_x_perm != -1) {
            ...
            for (int l = 0; l < stat_samples; l++) {
                ...
                double current_poro = count / (sub_x_size[iter_step] * sub_x_size[iter_step] * (Ny / Nz) * sub_x_size[iter_step] * (Nz / Nx));
                double mu = (MRT.tau - 0.5) / 3.f;
                stat_perm[l] = h * h * mu * current_poro * flow_rate / force_mag;
            }
        ...
        log_file = fopen("rev_analysis.csv", "a");
        fprintf(log_file, "%f %f %f %f %f %f %f %f \n", perm[iter_step], rev_x_perm, RE, CC, stat_CC, mean, stat_max, stat_min);
        fclose(log_file);

        // // tortuosity

        // if (iter_step >= 1 && iter_step <= samples - 1) // vale a pena calcular CC e RE toda vez ou if rev_x_poro != -1 ... ?
        // // e se eu fizer a primeira iteração fora do while pra não precisar comparar iterstep >=1?
        // {
        //     mean = 0;
        //     strd_dev = 0;
        //     double tort_count = 0;
        ...
        // {
        //     rev_x_tort = x_side * Dm->voxel_length;
        //     tort_rev = current_tort;
        // }
        ...
        //         if (vaz > 0)
        //             stat_tort[l] = sqrt(vax * vax + vay * vay + vaz * vaz) /vaz - 1;
        //         else
        //             stat_tort[l] = 1e100;
        //     }
        ...
        //     fprintf(log_file, "%f %f %f %f %f %f %f %f\n", tort[iter_step], rev_x_tort, RE, CC, stat_CC, mean, stat_max, stat_min);
        //     fclose(log_file);
        // //}
